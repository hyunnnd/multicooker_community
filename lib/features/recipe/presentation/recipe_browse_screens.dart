  ...
  Widget build(BuildContext context) {
    final items = const [
      ...
      _MethodItem(
        ...
        'Guided Cook',
        '단계별 안내 조리',
        '중간에 뒤집기·섞기·재료 추가가 필요한 레시피입니다.',
        ['삼겹살 구이', '마늘 버터 새우', '허브 스테이크'],
        figmaOrangeLight,
        'Guided',
      ),
      _MethodItem(
        ...
        'Quick Cook',
        '15분 이하 빠른 조리',
        '짧은 시간 안에 간단히 완성할 수 있는 레시피입니다.',
        ['10분 새우구이', '간단 볶음밥', '혼밥 스테이크'],
        figmaGreenLight,
        '간편',
      ),
      _MethodItem(
        ...
        'Professional',
        '온도·시간 세부 설정',
        '여러 단계의 온도와 시간을 세밀하게 조정하는 고급 레시피입니다.',
        ['닭갈비', '리조또', '전문가 스테이크'],
        figmaPurpleLight,
        'Professional',
      ),
        ...
        child: ListView(
          ...
          children: [
            ...
            const Padding(
              ...
              child: Text(
                '원하는 조리 방식에 맞춰 레시피를 찾아보세요.',
                style: TextStyle(fontSize: 14, color: figmaGray500),
              ),
            ),
            Padding(
                ...
                children: [
                  for (final item in items) ...[
                    _MethodCard(
                      item: item,
                      onTap: () => context.push(
                        '/recipes/browse?type=${Uri.encodeComponent(item.filter)}&title=${Uri.encodeComponent('조리 방식별 레시피')}',
                      ),
                    ),
...
class FoodTypeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final cats = [
      _FoodCat('구이', '🥩', _imageForTitle(recipes, '삼겹')),
      _FoodCat('볶음', '🥘', _imageForTitle(recipes, '닭갈비')),
      _FoodCat('솥밥', '🍚', _imageForTitle(recipes, '솥밥')),
      _FoodCat('찜', '♨️', _imageForTitle(recipes, '계란')),
      _FoodCat('간편식', '⚡', _imageForTitle(recipes, '새우')),
      _FoodCat('디저트', '🍮', null),
      _FoodCat('한식', '🥢', _imageForTitle(recipes, '고기')),
      _FoodCat('양식', '🍝', _imageForTitle(recipes, '리조또')),
    ];
    return Scaffold(
      backgroundColor: figmaBg,
      body: SafeArea(
                ...
                children: cats
                    .map(
                      (cat) => _FoodCatCard(
                        cat: cat,
                        onTap: () => context.push(
                          '/recipes/browse?type=${Uri.encodeComponent(cat.label)}&title=${Uri.encodeComponent('${cat.label} 레시피')}',
                        ),
                      ),
                    )
                    .toList(),
          ...
          children: [
            if (cat.image != null)
              ...
              Container(
                ...
                child: Text(cat.emoji, style: const TextStyle(fontSize: 36)),
              ),
            const DecoratedBox(
              decoration: BoxDecoration(
  ...
  Widget build(BuildContext context) {
    ...
    final themes = [
      _ThemeItem(
        ...
        '⏰',
        '빠르게 완성하는 간편 요리',
        _imageForTitle(recipes, '새우'),
      ),
      _ThemeItem(
        '재료만 넣고 끝',
        '🤖',
        'Full Auto 자동 조리',
        _imageForTitle(recipes, '계란'),
      ),
      _ThemeItem(
        '고기 굽기 좋은 날',
        '🥩',
        'Guided Cook 고기 특선',
        _imageForTitle(recipes, '삼겹'),
      ),
                ...
                children: [
                  for (final theme in themes) ...[
                    _ThemeCard(
                      theme: theme,
                      onTap: () => context.push(
                        '/recipes/browse?type=${Uri.encodeComponent(theme.label)}&title=${Uri.encodeComponent(theme.label)}',
                      ),
                    ),
                    const SizedBox(height: 12),
                  ],
          ...
          children: [
            SizedBox(
              ...
              child: theme.image == null
                  ...
                  : FigmaRecipeImage(source: theme.image),
            ),
            Expanded(
              child: Padding(
                padding: const EdgeInsets.symmetric(
...
class _RecipeBrowseListScreenState extends State<RecipeBrowseListScreen> {
  ...
  List<Recipe> _filtered(List<Recipe> all) {
    ...
    if (widget.type == 'Full Auto')
      list = list.where((r) => methodLabel(r) == 'Full Auto').toList();
    if (widget.type == 'Guided')
      list = list.where((r) => methodLabel(r) == 'Guided Cook').toList();
    if (widget.type == '간편')
      list = list.where((r) => r.totalTimeMin <= 15).toList();
    if (widget.type == '구이')
      list = list.where((r) => r.title.contains('구이')).toList();
    switch (_filter) {
      case '공식':
