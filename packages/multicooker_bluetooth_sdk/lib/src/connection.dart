...
class CookerConnection {
  ...
  Stream<BluetoothAdapterState> get onAdapterStateChanged =>
  ...
  void _onNotifyData(List<int> data) {
    ...
    while (_rxBuffer.isNotEmpty) {
      ...
      if (state != null) {
        debugPrint(
          '수신: status=${state.status}, temp=${state.currentTemperature}',
        ); // 임시 로그
  ...
  Future<void> _reconnect() async {
    ...
    for (
      var attempt = 0;
      attempt < 3 && _device != null && !_isConnected;
      attempt++
    ) {
